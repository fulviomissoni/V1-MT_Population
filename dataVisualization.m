...
%     mx = 1/(pi*sigma^4).*( 1 - 1/2.*(X)).*exp(-X/2);
    mx = 1/(2*pi*sigma^2)*exp(-X/2) - 1/(2*pi*K^2*sigma^2)*exp(-X/(2*K^2));
%     mx = exp(-X/2);
...
% for mytheta=1:param.nOrient
%     for indW=1:iteration+1
...
%             newtruetheta = repmat(truetheta(mytheta),[1 11]) + pi*(param.prefVel<0);
%             subplot(7,7,ind(i)), plot(param.prefVel, abs(rad2deg( newtruetheta - thetapop )))
%             ylim([0 200])
%             xlim([-2 2])
